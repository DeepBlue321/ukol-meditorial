        ...
        <div className="bg-white ">
          <div className="border-b border-gray-300 pb-8 flex flex-col gap-4 my-10">
            ...
            <div className="flex flex-col justify-start items-start mb-4">
                <h1 className="font-medium text-4xl font-lora">{post.translations[0].name}</h1>
              ...
              {parse(post.translations[0]?.content ?? '', {
                replace: (domNode) => {
                  if (isHTMLNode(domNode) && domNode.name === 'h2') {
                    return <h2 className="text-2xl mb-8 text-neutral-900 font-lora">{domNode.children?.map((child) => child.data).join('')}</h2>;
                  }
                  if (isHTMLNode(domNode) && domNode.name === 'p') {
                    return <p className="mb-8">{domNode.children?.map((child) => child.data).join('')}</p>;
                  }
                }
              })}
            </div>
